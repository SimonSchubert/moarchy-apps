#!/bin/bash
# Copy Contacts onto the phone, with shared/qs_ui vendored as ui/.
#
...
scp "$ROOT/manifest.json" "$ROOT/Contacts.qml" \
    "$ROOT/Contacts.js" "$ROOT/Store.js" "$ROOT/icon.svg" "$PHONE:$DEST/"
scp "$KIT/"*.qml "$KIT/"*.js "$KIT/qmldir" "$PHONE:$DEST/ui/"
scp "$ROOT/icon.svg" "$PHONE:.local/share/icons/hicolor/scalable/apps/org.moarchy.Contacts.svg"
scp "$ROOT/org.moarchy.Contacts.plugin.desktop" \
  "$PHONE:.local/share/applications/org.moarchy.Contacts.plugin.desktop"
...
ENDSSH
echo "==> done. tap Contacts in the drawer, or: omarchy-shell shell toggle $ID"
