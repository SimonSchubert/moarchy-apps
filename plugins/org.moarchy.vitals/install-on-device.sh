#!/bin/bash
# Copy Vitals onto the phone, with shared/qs_ui vendored as ui/.
#
#   plugins/org.moarchy.vitals/install-on-device.sh
#
...
scp "$ROOT/manifest.json" "$ROOT/Vitals.qml" "$ROOT/Sysinfo.js" "$ROOT/Collect.js" \
    "$ROOT/icon.svg" "$PHONE:$DEST/"
scp "$KIT/"*.qml "$KIT/"*.js "$KIT/qmldir" "$PHONE:$DEST/ui/"
scp "$ROOT/icon.svg" "$PHONE:.local/share/icons/hicolor/scalable/apps/org.moarchy.Vitals.svg"
scp "$ROOT/org.moarchy.Vitals.plugin.desktop" \
  "$PHONE:.local/share/applications/org.moarchy.Vitals.plugin.desktop"
...
ENDSSH
echo "==> done. tap Vitals in the drawer, or: omarchy-shell shell toggle $ID"
