#!/bin/bash
# Copy Weather onto the phone, with shared/qs_ui vendored as ui/.
#
...
scp "$ROOT/manifest.json" "$ROOT/Weather.qml" "$ROOT/Sky.qml" \
    "$ROOT/Forecast.js" "$ROOT/Store.js" \
    "$ROOT/icon.svg" "$PHONE:$DEST/"
scp "$KIT/"*.qml "$KIT/"*.js "$KIT/qmldir" "$PHONE:$DEST/ui/"
scp "$ROOT/icon.svg" "$PHONE:.local/share/icons/hicolor/scalable/apps/org.moarchy.Weather.svg"
scp "$ROOT/org.moarchy.Weather.plugin.desktop" \
  "$PHONE:.local/share/applications/org.moarchy.Weather.plugin.desktop"
...
ENDSSH
echo "==> done. tap Weather in the drawer, or: omarchy-shell shell toggle $ID"
