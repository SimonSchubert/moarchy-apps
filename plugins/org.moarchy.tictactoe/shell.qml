// Tictactoe as its own Quickshell process, for a system without omarchy-shell.
//
//   quickshell -p plugins/org.moarchy.tictactoe/shell.qml
//
// Inside the shell this file is never read: the plugin host instantiates
// Tictactoe.qml itself, keeps it loaded, and decides when the window is up. Here
...
ShellRoot {
  ...
  // MOARCHY_TICTACTOE_QUIT_AFTER=6 -- the check harness's clock, for the reason the
  // GTK app has one: a run that does not end is a run that hangs CI.
  Timer {
    interval: 1000 * parseInt(Quickshell.env("MOARCHY_TICTACTOE_QUIT_AFTER") || "0", 10)
    running: interval > 0
    onTriggered: Qt.quit()
  }
}
