#!/bin/bash
# Copy Tictactoe onto the phone, with shared/qs_ui vendored as ui/.
#
#   plugins/org.moarchy.tictactoe/install-on-device.sh
#
...
scp "$ROOT/manifest.json" "$ROOT/Tictactoe.qml" "$ROOT/Tictactoe.js"  "$ROOT/Store.js" \
    "$ROOT/icon.svg" "$PHONE:$DEST/"
scp "$KIT/"*.qml "$KIT/"*.js "$KIT/qmldir" "$PHONE:$DEST/ui/"
scp "$ROOT/icon.svg" "$PHONE:.local/share/icons/hicolor/scalable/apps/org.moarchy.TicTacToe.svg"
scp "$ROOT/org.moarchy.Tictactoe.plugin.desktop" \
  "$PHONE:.local/share/applications/org.moarchy.Tictactoe.plugin.desktop"
...
ENDSSH
echo "==> done. tap Tictactoe in the drawer, or: omarchy-shell shell toggle $ID"
