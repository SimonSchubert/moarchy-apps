#!/bin/bash
# Copy Habits onto the phone, with shared/qs_ui vendored as ui/.
#
#   plugins/org.moarchy.habits/install-on-device.sh
#
...
scp "$ROOT/manifest.json" "$ROOT/Habits.qml" "$ROOT/Habits.js" "$ROOT/Game.js" \
    "$ROOT/icon.svg" "$PHONE:$DEST/"
scp "$KIT/"*.qml "$KIT/"*.js "$KIT/qmldir" "$PHONE:$DEST/ui/"
scp "$ROOT/icon.svg" "$PHONE:.local/share/icons/hicolor/scalable/apps/org.moarchy.Habits.svg"
scp "$ROOT/org.moarchy.Habits.plugin.desktop" \
  "$PHONE:.local/share/applications/org.moarchy.Habits.plugin.desktop"
...
ENDSSH
echo "==> done. tap Habits in the drawer, or: omarchy-shell shell toggle $ID"
