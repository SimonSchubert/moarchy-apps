#!/bin/bash
# Copy Calculator onto the phone, with shared/qs_ui vendored as ui/.
#
...
scp "$ROOT/manifest.json" "$ROOT/Calculator.qml" "$ROOT/Calc.js" "$ROOT/Decimal.js" \
    "$ROOT/Store.js" "$ROOT/icon.svg" "$PHONE:$DEST/"
scp "$KIT/"*.qml "$KIT/"*.js "$KIT/qmldir" "$PHONE:$DEST/ui/"
scp "$ROOT/icon.svg" "$PHONE:.local/share/icons/hicolor/scalable/apps/org.moarchy.Calculator.svg"
scp "$ROOT/org.moarchy.Calculator.plugin.desktop" \
  "$PHONE:.local/share/applications/org.moarchy.Calculator.plugin.desktop"
...
ENDSSH
echo "==> done. tap Calculator in the drawer, or: omarchy-shell shell toggle $ID"
