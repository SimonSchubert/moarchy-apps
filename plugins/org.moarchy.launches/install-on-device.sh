#!/bin/bash
# Copy Launches onto the phone, with shared/qs_ui vendored as ui/.
#
...
scp "$ROOT/manifest.json" "$ROOT/Launches.qml" "$ROOT/Launches.js" "$ROOT/Store.js" \
    "$ROOT/icon.svg" "$PHONE:$DEST/"
scp "$KIT/"*.qml "$KIT/"*.js "$KIT/qmldir" "$PHONE:$DEST/ui/"
scp "$ROOT/icon.svg" "$PHONE:.local/share/icons/hicolor/scalable/apps/org.moarchy.Launches.svg"
scp "$ROOT/org.moarchy.Launches.plugin.desktop" \
  "$PHONE:.local/share/applications/org.moarchy.Launches.plugin.desktop"
