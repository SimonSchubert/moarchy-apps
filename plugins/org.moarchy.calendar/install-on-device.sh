#!/bin/bash
# Copy Calendar onto the phone, with shared/qs_ui vendored as ui/.
#
...
scp "$ROOT/manifest.json" "$ROOT/Calendar.qml" "$ROOT/Month.qml" "$ROOT/EventRow.qml" \
    "$ROOT/Dates.js" "$ROOT/Events.js" "$ROOT/Store.js" "$ROOT/icon.svg" "$PHONE:$DEST/"
scp "$KIT/"*.qml "$KIT/"*.js "$KIT/qmldir" "$PHONE:$DEST/ui/"
scp "$ROOT/icon.svg" "$PHONE:.local/share/icons/hicolor/scalable/apps/org.moarchy.Calendar.svg"
scp "$ROOT/org.moarchy.Calendar.plugin.desktop" \
  "$PHONE:.local/share/applications/org.moarchy.Calendar.plugin.desktop"
...
ENDSSH
echo "==> done. tap Calendar in the drawer, or: omarchy-shell shell toggle $ID"
