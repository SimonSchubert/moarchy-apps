#!/bin/bash
# Copy Coins onto the phone, with shared/qs_ui vendored as ui/.
#
#   plugins/org.moarchy.coins/install-on-device.sh
#
...
scp "$ROOT/manifest.json" "$ROOT/Coins.qml" "$ROOT/Market.js" "$ROOT/Store.js" \
    "$ROOT/icon.svg" "$PHONE:$DEST/"
scp "$KIT/"*.qml "$KIT/"*.js "$KIT/qmldir" "$PHONE:$DEST/ui/"
scp "$ROOT/icon.svg" "$PHONE:.local/share/icons/hicolor/scalable/apps/org.moarchy.Coins.svg"
scp "$ROOT/org.moarchy.Coins.plugin.desktop" \
  "$PHONE:.local/share/applications/org.moarchy.Coins.plugin.desktop"
...
ENDSSH
echo "==> done. tap Coins in the drawer, or: omarchy-shell shell toggle $ID"
