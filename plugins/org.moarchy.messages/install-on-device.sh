...
scp "$ROOT/manifest.json" "$ROOT/Messages.qml" \
    "$ROOT/Modem.js" "$ROOT/Numbers.js" "$ROOT/Store.js" "$ROOT/Threads.js" "$ROOT/icon.svg" "$PHONE:$DEST/"
scp "$KIT/"*.qml "$KIT/"*.js "$KIT/qmldir" "$PHONE:$DEST/ui/"
scp "$ROOT/icon.svg" "$PHONE:.local/share/icons/hicolor/scalable/apps/org.moarchy.Messages.svg"
scp "$ROOT/org.moarchy.Messages.plugin.desktop" \
  "$PHONE:.local/share/applications/org.moarchy.Messages.plugin.desktop"
...
# Chatty's daemon takes every text off the modem and deletes it, so while it
# runs nothing arrives here. Masked rather than disabled, because the moarchy
# package links it into default.target.wants where `disable` cannot reach it.
if systemctl --user cat sm.puri.Chatty-daemon.service >/dev/null 2>&1; then
  systemctl --user stop sm.puri.Chatty-daemon.service 2>/dev/null || true
  systemctl --user mask sm.puri.Chatty-daemon.service >/dev/null 2>&1 \
    && echo "    masked sm.puri.Chatty-daemon (undo: systemctl --user unmask sm.puri.Chatty-daemon.service)"
fi
