#!/bin/bash
# Copy Reversi onto the phone, with shared/qs_ui vendored as ui/.
#
#   plugins/org.moarchy.reversi/install-on-device.sh
#
...
scp "$ROOT/manifest.json" "$ROOT/Reversi.qml" "$ROOT/Reversi.js" "$ROOT/Bits.js" "$ROOT/Ai.js" "$ROOT/Store.js" "$ROOT/search.js" \
    "$ROOT/icon.svg" "$PHONE:$DEST/"
scp "$KIT/"*.qml "$KIT/"*.js "$KIT/qmldir" "$PHONE:$DEST/ui/"
scp "$ROOT/icon.svg" "$PHONE:.local/share/icons/hicolor/scalable/apps/org.moarchy.Reversi.svg"
scp "$ROOT/org.moarchy.Reversi.plugin.desktop" \
  "$PHONE:.local/share/applications/org.moarchy.Reversi.plugin.desktop"
...
ENDSSH
echo "==> done. tap Reversi in the drawer, or: omarchy-shell shell toggle $ID"
