...
# How a card is drawn, as fractions of one. The geometry -- where a card *is* --
# lives in layout.py, because where a card is drawn and where a tap lands are
# the same function and that is worth testing without a display.
MORTAR = 0.06
...
class TableView(Gtk.DrawingArea):
    ...
    def _recycle(self, cr, layout: Layout) -> None:
        ...
        """
        x = layout.slot_x(column_of(STOCK))
