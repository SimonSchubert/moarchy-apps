...
As in Reversi, a reload is not the whole mechanism: almost the entire screen is
drawn rather than styled, and cairo cannot read a CSS class. So the palette is
handed to the window as well, which hands it to the table, which repaints.
Everything that *is* a widget -- the banner, the buttons, the record -- comes
from the stylesheet and needs nothing but the reload.
"""
...
from .window import SolitaireWindow  # noqa: E402

APP_ID = "org.moarchy.Solitaire"
...
class SolitaireApplication(Adw.Application):
    ...
    def do_activate(self) -> None:
        ...
        if window is None:
            window = SolitaireWindow(self.store, application=self)
        ...
        seconds = os.environ.get("MOARCHY_SOLITAIRE_QUIT_AFTER")
    ...
    def do_shutdown(self) -> None:
        ...
        try:
            ...
            print(f"moarchy-solitaire: could not save the game: {exc}", file=sys.stderr)
    ...
    def _on_about(self, *_args) -> None:
        about = Adw.AboutDialog(
            ...
            comments=(
                "Klondike for a Linux phone. A tap moves the card when there "
                "is one place for it to go and asks when there is more than "
                "one, and the app says when a deal has nothing left in it."
            ),
        )
...
def main() -> int:
    return SolitaireApplication().run(sys.argv)
