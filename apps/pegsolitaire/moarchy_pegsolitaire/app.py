...
from .window import PegSolitaireWindow  # noqa: E402

APP_ID = "org.moarchy.PegSolitaire"
...
class PegSolitaireApplication(Adw.Application):
    ...
    def do_activate(self) -> None:
        ...
        if window is None:
            window = PegSolitaireWindow(self.store, application=self)
        ...
        seconds = os.environ.get("MOARCHY_PEGSOLITAIRE_QUIT_AFTER")
    ...
    def do_shutdown(self) -> None:
        ...
        try:
            ...
            print(
                f"moarchy-pegsolitaire: could not save the game: {exc}", file=sys.stderr
            )
    ...
    def _on_about(self, *_args) -> None:
        about = Adw.AboutDialog(
            application_name="Peg Solitaire",
            ...
            comments=(
                "Peg solitaire for a Linux phone. Nine figures, every one of "
                "them solvable, and a hint that runs the solver rather than a "
                "heuristic — so it can tell you when there is no way left."
            ),
        )
...
def main() -> int:
    return PegSolitaireApplication().run(sys.argv)
