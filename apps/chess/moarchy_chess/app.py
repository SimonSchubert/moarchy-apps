...
Unlike most of the apps here, a reload is not the whole mechanism: most of this
app is drawn rather than styled, and cairo cannot read a CSS class. So the
palette is handed to the window as well, which hands it to the board and to the
two names above it, all of which repaint. Everything that *is* a widget -- the
buttons, the rows, the record -- still comes from the stylesheet and needs
...
from .window import ChessWindow  # noqa: E402

APP_ID = "org.moarchy.Chess"
...
class ChessApplication(Adw.Application):
    ...
    def do_activate(self) -> None:
        ...
        if window is None:
            window = ChessWindow(self.store, application=self)
        ...
        seconds = os.environ.get("MOARCHY_CHESS_QUIT_AFTER")
    ...
    def do_shutdown(self) -> None:
        ...
        try:
            ...
            print(f"moarchy-chess: could not save the game: {exc}", file=sys.stderr)
    ...
    def _apply_styles(self) -> None:
        ...
        if self._styles is None:
            ...
            # somebody who has written one means it.
    ...
    def _on_about(self, *_args) -> None:
        about = Adw.AboutDialog(
            ...
            comments=(
                "Chess for a Linux phone. The board is your theme's colours, "
        ...
        about.add_legal_section(
            "Pieces",
            "Font Awesome Free 5 chess icons",
            Gtk.License.CUSTOM,
...
def main() -> int:
    return ChessApplication().run(sys.argv)
