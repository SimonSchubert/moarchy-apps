...
As in Reversi, a reload is not the whole mechanism: the board is drawn rather
than styled, and cairo cannot read a CSS class. So the palette is handed to the
window as well, which hands it to the board, which repaints. Everything that
*is* a widget -- the chips, the buttons, the record -- comes from the stylesheet
and needs nothing but the reload.

The one thing this app does at startup that the others do not is solve the game.
It costs about a tenth of a second on an A53, it is wanted exactly once, and it
is spent here -- on a thread, while somebody is looking at an empty board --
rather than in the middle of the reply to their first tap.
"""
...
from . import __version__, ai, theme  # noqa: E402
from .store import Store  # noqa: E402
from .window import TicTacToeWindow  # noqa: E402

APP_ID = "org.moarchy.TicTacToe"
...
class TicTacToeApplication(Adw.Application):
    ...
    def do_startup(self) -> None:
        ...
        # Daemon, so quitting never waits for it. If a tap beats the thread to
        # the table, `ai.choose` simply does the work itself: the values are
        # deterministic, so the worst a race costs is the same answer computed
        # twice.
        threading.Thread(target=ai.warm, daemon=True).start()
    ...
    def do_activate(self) -> None:
        ...
        if window is None:
            window = TicTacToeWindow(self.store, application=self)
        ...
        seconds = os.environ.get("MOARCHY_TICTACTOE_QUIT_AFTER")
        if seconds and seconds.isdigit():
            GLib.timeout_add_seconds(int(seconds), self.quit)

    def do_shutdown(self) -> None:
        # Belt and braces: the window saves after every mark and again on
        # close-request, but a session ending under us never sends one.
        try:
            self.store.save()
        except OSError as exc:
            print(f"moarchy-tictactoe: could not save the game: {exc}", file=sys.stderr)
    ...
    def _on_about(self, *_args) -> None:
        about = Adw.AboutDialog(
            ...
            comments=(
                "Noughts and crosses for a Linux phone. The game is solved, so "
                "the levels are about what the opponent notices rather than how "
                "deep it looks, and the marks swap every rematch."
            ),
        )
...
def main() -> int:
    return TicTacToeApplication().run(sys.argv)
