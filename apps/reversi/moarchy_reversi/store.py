...
from .reversi import DARK, LIGHT, Game
...
def data_dir() -> Path:
    ...
    override = os.environ.get("MOARCHY_REVERSI_DIR")
    if override:
        return Path(override)
    base = os.environ.get("XDG_DATA_HOME") or str(Path.home() / ".local" / "share")
    return Path(base) / "moarchy-reversi"
...
class Store:
    ...
    def __init__(self, path: Path | str | None = None) -> None:
        self.path = Path(path) if path else data_dir() / "reversi.json"
        self.moves: list[int] = []
        self.mode: str = SOLO
    ...
    def load(self) -> None:
        ...
        if isinstance(game, dict):
            ...
            self.human = LIGHT if game.get("human") == "light" else DARK
    ...
    def save(self) -> None:
        ...
        payload = {
            ...
            "game": {
                "mode": self.mode,
                "level": self.level,
                "human": "dark" if self.human == DARK else "light",
    ...
    def begin(self, *, mode: str, level: str, human: int) -> Game:
        ...
        self.human = human if human in (DARK, LIGHT) else DARK
        self.moves = []
        self.finished = False
        return Game()
