...
`omarchy-theme-set` recolours the marks in place -- the same behaviour the bar,
the drawer and the keyboard have. Mark colours are CSS classes, so a reload is
the whole mechanism: every mark already on screen repaints and the grids never
...
from .habits import Store  # noqa: E402
from .window import HabitsWindow  # noqa: E402

APP_ID = "org.moarchy.Habits"
...
class HabitsApplication(Adw.Application):
    ...
    def do_activate(self) -> None:
        ...
        if window is None:
            window = HabitsWindow(self.store, application=self)
        window.present()
        ...
        seconds = os.environ.get("MOARCHY_HABITS_QUIT_AFTER")
    ...
    def do_shutdown(self) -> None:
        ...
        try:
            ...
            print(f"moarchy-habits: could not save habits: {exc}", file=sys.stderr)
    ...
    def _on_about(self, *_args) -> None:
        about = Adw.AboutDialog(
            application_name="Habits",
            application_icon=APP_ID,
            version=__version__,
            developer_name="Simon Schubert",
            license_type=Gtk.License.MIT_X11,
            website="https://github.com/SimonSchubert/moarchy-habits",
            issue_url="https://github.com/SimonSchubert/moarchy-habits/issues",
            comments=(
                "Habit tracking for a Linux phone. Every day is a tap, the "
                "history is one file, and the colours come from your theme."
            ),
        )
...
def main() -> int:
    return HabitsApplication().run(sys.argv)
