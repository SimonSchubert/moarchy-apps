"""The application: styles, one sampler, and one window.

The stylesheet is rebuilt whenever the active Omarchy theme changes, so
`omarchy-theme-set` recolours the meters in place -- the same behaviour the bar,
the drawer and the keyboard have. As in Reversi and Solitaire a reload is not
the whole mechanism: the graphs and the core bars are drawn with cairo, which
cannot read a CSS class, so the palette is handed to the window as well.

There is no store. This app has nothing to save: every number in it is a fact
about the machine a moment ago, and the one piece of state it could keep -- which
page you were on -- is not worth a file on a phone that reopens apps cold.

The sampler is made here rather than in the window because *where /proc is* is a
property of the process, not of a window: it is read once, from the environment,
so that the screenshot harness and the tests can point the whole app at a
directory of files instead of at the running kernel.
"""
...
from .sysinfo import Sampler, sysroot_for  # noqa: E402
from .window import VitalsWindow  # noqa: E402

APP_ID = "org.moarchy.Vitals"
...
class VitalsApplication(Adw.Application):
    def __init__(self) -> None:
        ...
        # MOARCHY_VITALS_DIR is the harness pointing the app at a directory
        # shaped like /proc. Unset -- which is every real run -- it reads the
        # kernel's own.
        self.sampler = Sampler(sysroot_for(os.environ.get("MOARCHY_VITALS_DIR")))
    ...
    def do_activate(self) -> None:
        ...
        if window is None:
            window = VitalsWindow(self.sampler, application=self)
        ...
        seconds = os.environ.get("MOARCHY_VITALS_QUIT_AFTER")
    ...
    def _on_about(self, *_args) -> None:
        about = Adw.AboutDialog(
            ...
            comments=(
                "A task manager for a Linux phone: what the processor, the "
                "memory, the storage and the network are doing, what is doing "
                "it, and one tap to stop it. Everything is read from /proc, "
                "nothing leaves the device, and the colours come from your theme."
            ),
        )
...
def main() -> int:
    return VitalsApplication().run(sys.argv)
