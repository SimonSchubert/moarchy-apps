...
from .window import MinesweeperWindow  # noqa: E402

APP_ID = "org.moarchy.Minesweeper"
...
class MinesweeperApplication(Adw.Application):
    ...
    def do_activate(self) -> None:
        ...
        if window is None:
            window = MinesweeperWindow(self.store, application=self)
        ...
        seconds = os.environ.get("MOARCHY_MINESWEEPER_QUIT_AFTER")
    ...
    def do_shutdown(self) -> None:
        ...
        try:
            ...
            print(
                f"moarchy-minesweeper: could not save the game: {exc}", file=sys.stderr
            )
    ...
    def _on_about(self, *_args) -> None:
        about = Adw.AboutDialog(
            ...
            comments=(
                "Minesweeper for a Linux phone. Boards shaped for a portrait "
                "screen, a flag button that latches, and a clock that stops "
                "when the app is not on it."
            ),
        )
...
def main() -> int:
    return MinesweeperApplication().run(sys.argv)
