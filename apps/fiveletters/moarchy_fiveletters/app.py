...
*is* a widget -- the banner, the buttons, the record -- comes from the
stylesheet and needs nothing but the reload.
"""
...
from .window import FiveLettersWindow  # noqa: E402
from .words import Words  # noqa: E402

APP_ID = "org.moarchy.FiveLetters"
...
class FiveLettersApplication(Adw.Application):
    ...
    def do_activate(self) -> None:
        ...
        if window is None:
            window = FiveLettersWindow(self.store, self.words, application=self)
        ...
        seconds = os.environ.get("MOARCHY_FIVELETTERS_QUIT_AFTER")
    ...
    def do_shutdown(self) -> None:
        ...
        try:
            ...
            print(
                f"moarchy-fiveletters: could not save the game: {exc}", file=sys.stderr
            )
    ...
    def _on_about(self, *_args) -> None:
        about = Adw.AboutDialog(
            ...
            comments=(
                "A five-letter word a day, for a Linux phone. Its own keyboard "
                "so the board is never covered, a mark on every tile as well as "
                "a colour, and English only — a word list cannot be translated."
            ),
        )
...
def main() -> int:
    return FiveLettersApplication().run(sys.argv)
